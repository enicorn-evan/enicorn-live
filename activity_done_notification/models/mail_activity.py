...
class MailActivityExt(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        body_template = self.env.ref('activity_done_notification.message_activity_done')
        for activity in self:
            model_description = self.env['ir.model']._get(activity.res_model).display_name
            body = body_template.render(
                dict(activity=activity, model_description=model_description, assignee=self.env.user.name),
                engine='ir.qweb',
        ...
        return super(MailActivityExt, self)._action_done()
